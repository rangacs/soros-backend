...
class UserGroupsAccess extends CActiveRecord
{
	...
	public static function homeList()
	{
		...
		return self::fixUrlNames(self::$_rawData,"F_CH_TO_LOWER");
	}

	
	/** 
	 * returns the filtered array list based on the $filterString
	 * Possible Input filters:
	 * F_CH_TO_LOWER => Convert First character to Lowercase
	 * F_CH_TO_UPPER => Convert First character to Uppercase	 
	 * @return array
	 */	
	public static function fixUrlNames($kvalPairs,$filterString="F_CH_TO_LOWER")
	{
		$tempArray = array();
		if(is_array($tempArray))
		{
			if($filterString == "F_CH_TO_LOWER")
			{
				foreach($kvalPairs as $id=>$val)
				{
					$id{0} = strtolower($id{0});
					if($id{0} == "/")
						$id{1} = strtolower($id{1});

					$tempArray = self::array_push_assoc($tempArray, $id,$val);
				}
			}
		}
		return $tempArray;
