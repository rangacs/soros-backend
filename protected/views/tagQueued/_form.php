...
<div class="form">
    ...
    $startDate = date('Y-m-d',strtotime($model->LocalstartTime));
    $startTime = date("H:i",strtotime($model->LocalstartTime));
    
    if(!$_REQUEST["stg"]) {
        $endDate   = date('Y-m-d',strtotime($model->LocalendTime));
        $endTime   = date("H:i",strtotime($model->LocalendTime));
    }else {
        $stg = 1;
    ...
    <div class="clearfix">
        ...
        <div class="form-input">
            ...
            <?php echo CHtml::submitButton($model->isNewRecord ? Yii::t('app', 'Create') : (($stg)?Yii::t('app', 'STOP'):Yii::t('app', 'Save')), 
            array('id' => 'subMB')); ?>
        </div>
    </div>
    <?php $this->endWidget(); ?>
