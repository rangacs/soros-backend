...
//end createDisplay()..
    ...
    public static function createDisplay_renderLogic($portlets, $gridIds, $columns) {
        $jsIncluded = 0;
        $jsTIncluded = 0;
	      ...
	      var IL_callback = {
		     ...
		     IL_execute : function(){
					 ...
					 //SHOW FIRST RECORD to our audience upon page load..
					  $.ajax({
						type: "POST",
						//url: "/Helios/dash/Dash",     //UIDashboardController::Dash()..
						url: "' . Yii::app()->baseUrl . '/dash/Dash",
						data: {"ajaxForwardRequest" : fetchDashboardElements, "gadgetType" : gadgetType, "widgetsPos" : widgetsPos},
						success: function(stringified)
						{
	...
	';
                        ...
                        $pT = isset($_REQUEST['pageType']) ? $_REQUEST['pageType'] : '';
                        if ((isset($_REQUEST['pageType'])) && ($_REQUEST['pageType'] == "timeRange") && (isset($_REQUEST['sTime'])) && (isset($_REQUEST['eTime']))) {
                        ...
                        } elseif ((@$_REQUEST['pageType'] === "tonsRange")) {




                            $startTimeT = strtotime($_REQUEST['sTime']);
                            $startTimeS = date("Y-m-d H:i:s", $startTimeT);

                            $curTime = strtotime($_REQUEST['eTime']);

                            $endTimeS = date("Y-m-d H:i:s", $curTime);
                            ...
                            $timerange = DashHelper::getTonsTImeRange($totlTons, $startTimeS, $endTimeS);

                            $prevTime = $timerange['LocalstartTime'];
                            $curTime = $timerange['LocalendTime'];

                            $pageType = "tonsRange";
                            $postString = '"pageType":"tonsRange", "sTime": "' . $prevTime . '", "eTime" : "' . $curTime . '"';
                        }
                        ...
                        $anInfoFor = "Analysis Information for ";
                        echo '<script type="text/javascript">
	...
	if(groupStyle != "")
	{
		...
		if(responseSplitAr[2]){
		    ...
		    for(var lk=6,oi=0;lk<responseSplitAr.length;lk++,oi++)
		    {
				...
				if(elemNameTmp == "C3A")
					cusSpVal = (setpointsGlobalAr.C3A);
				if(elemNameTmp == "KH")
					cusSpVal = (setpointsGlobalAr.KH);
				if(elemNameTmp == "SM")
					cusSpVal = (setpointsGlobalAr.SM);
				if(elemNameTmp == "IM")
					cusSpVal = (setpointsGlobalAr.IM);
				if(elemNameTmp == "AM")
					cusSpVal = (setpointsGlobalAr.AM);
				if(elemNameTmp == "CaO")
					cusSpVal = (setpointsGlobalAr.CaO);
				if(elemNameTmp == "Al2O3")
					cusSpVal = (setpointsGlobalAr.Al2O3);
				if(elemNameTmp == "Fe2O3")
					cusSpVal = (setpointsGlobalAr.Fe2O3);
				if(elemNameTmp == "SiO2")
					cusSpVal = (setpointsGlobalAr.SiO2);
	...
	if ((chartStyle =="simple") || (chartStyle =="ddspline"))
	{
		var oi=0;
		...
		$("#containerHC_' . $j . '").highcharts("StockChart",
		{
		    ...
		    title: {
				text: "' . Yii::t("app", $anInfoFor) . '"+ elementsArList[oi].name
			},
		...
		$("#containerHC_' . $j . '").highcharts({
			title: {
				text: "Analysis Results for :" + elemTitleSubAr,
				x: -20 //center
			},
			subtitle: {
    ...
    /* }); //end document.ready.. */
                    ...
                    if (($divIdVal1 == "System_Messages")) {
                        ...
                        if ($j == 0 && strtolower($controllerId) == strtolower('dash')) {
                            ...
                            $aTag = $aTag . '<a href="#average-table" style="float:right;margin-right:5px;margin-bottom:5px;" for="button-toggle2-3" aria-pressed="false" class="ui-button ui-widget ui-state-default ui-button-text-only ui-corner-right" role="button" aria-disabled="false"><span class="ui-button-text">' . Yii::t('dash', 'Avg Table') . '</span></a>';
                            $aTag = $aTag . ' <label id="realTimeLink" style="float:right;margin-right:5px;margin-bottom:5px;" for="button-toggle2-3" aria-pressed="false" class="ui-button ui-widget ui-state-default ui-button-text-only ui-corner-right" role="button" aria-disabled="false"><span class="ui-button-text">' . Yii::t('dash', 'Real-Time') . '</span></label>';
                        } else if ($j == 1) {
                            $title = "Lab Results";
                        } else if ($j == 2) {
		   ...
		   <div  style="' . $heightPx . '" class="grid_' . $gridColVal1 . ' portlet ui-sortable clearfix padMargin collapsible" draggable="true" id="' . $row["id"] . '_' . $j . '">
			<header>
				<h2>
						' . Yii::t('dash', $title) . '   ' . $curTime . '
				</h2>
			</header>
		      <section id = "section_' . $j . '" class="no-padding clearfix" >';

                        $downloadLink = Yii::app()->createAbsoluteUrl("export/analysis", array('startDate' => $prevTime, 'endDate' => $curTime));
                        echo "<a  class='pull-right DTTT_button' href='$downloadLink'>Export</a>";
                        echo '
...
//end createDisplay_renderLogic()..
    ...
    public static function getMeChartsInfo($widgetsPos, $uid, $pageType, $default, $sTime, $eTime, $tag = NULL, $tGrp = NULL, $tagStatus = NULL) {
        ...
        if (($pageType == "timeRange") && (isset($sTime)) && (isset($eTime))) {
            $prevTime = ($sTime);
            $curTime = ($eTime);
        } elseif (($pageType == "tonsRange") && (isset($sTime)) && (isset($eTime))) {
            $prevTime = ($sTime);
            $curTime = ($eTime);
        ...
        if ($tGrpSet) {
            ...
            $tableNameQuery = "SELECT DB_ID_string FROM `rta_config_master` rcm WHERE rcm.`rtaMasterID` IN (SELECT tag.`rtaMasterID` FROM $tagStatTable tag WHERE tagID=$tagId)";
            ...
            if ('queued' == $tagStatus) {
                ...
                $tag = TagQueued::model()->findByPk($tagId);
            } else {
                $tag = TagCompleted::model()->findByPk($tagId);
            }
            $prevTime = $tag['LocalstartTime'];
            $curTime = $tag['LocalendTime'];

            $cntRows = Yii::app()->db->createCommand("SELECT count(dataID) as cnt FROM `$analysisTableName`  WHERE LocalendTime BETWEEN( '$prevTime') AND ('$curTime')")->queryRow();
...
//if($rs_k2 == 'endTic')
        ...
        if ($last2 == "SM" || $last2 == "IM" || $last2 == "KH")
            $sp_name_val = $last2;
        if ($last3 == "LSF" || $last3 == "C3S")
            $sp_name_val = $last3;
        if ($last4 == "NAEQ")
            $sp_name_val = $last4;
...
//getMeChartsInfo
    ...
    public static function getMeTablesInfo($widgetsPos, $uid, $pageType, $default, $sTime, $eTime, $tag = NULL, $tGrp = NULL, $tagStatus = NULL) {
        ...
        if (isset($_REQUEST['sTime']) && isset($_REQUEST['eTime'])) {
            ...
            $curTime = date("Y-m-d H:i:s", strtotime($_REQUEST['eTime']));
        } else {
            $curTime = date("Y-m-d H:i:s");
            $curTimeSt = strtotime($curTime);
        ...
        $theadStr .= '<th class="ui-state-default" style="width:' . $widthStr . '%;" ><div class="DataTables_sort_wrapper" >' . Yii::t('app', 'Time-Stamp') . '</div></th>';
        ...
        foreach ($elem_list as $eval) {
            ...
            if (!empty($eval)) {
                ...
                if (substr($eval, 0, 4) == "src_") {
                    ...
                    if (isset($srcNameModAr[$eval]))
                        $eval = $srcNameModAr[$eval];
                }
        ...
        if (($pageType == "timeRange") && (isset($sTime)) && (isset($eTime))) {
            ...
            $totlTons = $_REQUEST['totalTons'];
            $timerange = DashHelper::getTonsTImeRange($totlTons, $sTime, $eTime);
...
//                            $date = new DateTime($rs_v2);
                            $trs_v2 = date($dateTimeFormat, strtotime($rs_v2));
                            //$trs_v2 = date("m/d/y h:i:s",$rs_v2);
                            $tbodyStr .= '<td style="width: 160px;">' . $trs_v2 . '</td>';
                        } else if ($rs_v2 != ' ') {
                            $tbodyStr .= '<td >' . round($rs_v2, 2) . '</td>';
                    ...
                    if (($detector_source == "rm_source_feeder_inputs")) {

                        $sr1M = $rs_v1["src_1_r"] == 0 ? $rs_v1["src_1_m"] : $rs_v1["src_1_m"];
                        $sr2M = $rs_v1["src_2_r"] == 0 ? $rs_v1["src_2_m"] : $rs_v1["src_2_m"];
                        $sr3M = $rs_v1["src_3_r"] == 0 ? $rs_v1["src_3_m"] : $rs_v1["src_3_m"];
                        $sr4M = $rs_v1["src_4_r"] == 0 ? $rs_v1["src_4_m"] : $rs_v1["src_4_m"];
                        $sr5M = $rs_v1["src_5_r"] == 0 ? $rs_v1["src_5_m"] : $rs_v1["src_5_m"];
                        $sr6M = $rs_v1["src_6_r"] == 0 ? $rs_v1["src_6_m"] : $rs_v1["src_6_m"];
                        $sr7M = $rs_v1["src_7_r"] == 0 ? $rs_v1["src_7_m"] : $rs_v1["src_7_m"];
                        $sr8M = $rs_v1["src_8_r"] == 0 ? $rs_v1["src_8_m"] : $rs_v1["src_8_m"];


                        $tsumFdr = round(($sr1M + $sr2M + $sr3M + $sr4M + $sr5M + $sr6M + $sr7M + $sr8M), 2);
                        $tbodyStr .= '<td >' . $tsumFdr . '</td>';
                        if ($tsumFdr > 0) {
                            $avgArr["R_TPH"] = $avgArr["R_TPH"] + $tsumFdr;
                            $avgInd["R_TPH"] = $avgInd["R_TPH"] + 1;
                        }
                    }
                    $tbodyStr .= '</tr>';
                }
        ...
        if (isset($avgArr) && count($avgArr) > 0) {
...
//getMeIndChartAvgInfo
    ...
    public static function getMeIndChartInfo($element_name, $givenTime = "", $sim = 0) {
        ...
        if (isset($_REQUEST['start_date']) && isset($_REQUEST['end_date'])) {
            $prevTime = date('Y-m-d H:i:s', strtotime($_REQUEST['start_date']));
            $curTime = date('Y-m-d H:i:s', strtotime($_REQUEST['end_date']));
        } else if (strtotime($givenTime) > 0) {
            $curTime = $givenTime;
            $curTimeSt = strtotime($givenTime);
            $temp_curTimest = $curTimeSt;
            $curTimeSt = $curTimeSt + (60 * 60); //Go back 15 mins worth Information
            ...
            $prevTimeSt = $curTimeSt - (8 * 60 * 60); //60 mins worth Information
            $prevTime = date("Y-m-d H:i:s", $prevTimeSt);
        }

        if ($element_name) {
            if ($element_name == "KH")
                $element_name = "IF( (CaO <= 0), 0 , ROUND(((CaO - 1.65 * Al2O3 - 0.35 * Fe2O3) / (2.80 * sio2)),2) ) as KH";
            else if ($element_name == "LSF")
                $element_name = "IF( (CaO <= 0), 0 , ROUND((CaO/(2.8*SiO2+1.18*Al2O3+0.65*Fe2O3) * 100),2) ) as LSF";
            else if ($element_name == "C3S")
                $element_name = "IF( (CaO <= 0), 0 , ROUND((4.07*CaO - 7.6*SiO2 - 6.72*Al2O3 - 1.43*Fe2O3 - 2.852*SO3 ),2) ) as C3S";
            else if ($element_name == "C3A")
                $element_name = "IF( (Al2O3<= 0), 0 , ROUND((Al2O3 * 2.65 - 1.69 * Fe2O3),2) ) as C3A";
            else if ($element_name == "NAEQ")
                $element_name = "IF( (K2O <= 0), 0 , ROUND((K2O * 0.658 + Na2O ),2) ) as NAEQ";
            else if ($element_name == "SM")
                $element_name = "IF( (SiO2 <= 0), 0 , ROUND((SiO2/(Al2O3+Fe2O3)),2) ) as SM";
            else if ($element_name == "IM")
                $element_name = "IF( (Al2O3 <= 0), 0 , ROUND((Al2O3/Fe2O3),2) ) as IM";
            else
...
//getMeIndvChart
    ...
    public static function createDetInfoUls() {
        ...
        $detInfoArray_keys = array("Daemon_Running" => Yii::t('app', "System"),
            "Detector_Temp" => Yii::t('app', "Temperature"),
            "Count_Rate_(aligned)" => Yii::t('app', "Count Rate"),
            "Good_Data_Secs" => Yii::t('app', "Data Quality"),
            "H_Peak_Raw_Channel" => Yii::t('app', "H Peak"),
            "PMT_Voltage_Readback" => Yii::t('app', "HV")
        );
...
//function
    public static function validateAndSetAnalyzerRecordUsingRange($row, $acSettings) {

        $rowValid = true;
        $acElements = array_keys($acSettings);

        //Find if any value is > 0 
        foreach ($row as $ele => $eleValue) {
            if (in_array($ele, $acElements)) {
                $eleSettings = isset($acSettings[$ele]) ? $acSettings[$ele] : array();
                $min = $eleSettings['min'] - $eleSettings['valid_datapoint_range'];
                $max = $eleSettings['max'] + $eleSettings['valid_datapoint_range'];
                if ($eleValue < $min || $eleValue > $max)
                    $rowValid = false;
            }
        }

        //drop whole row if any value is invalid
        $newRow = array();
        foreach ($row as $ele => $eleValue) {
            if ($rowValid)
                $newRow[$ele] = $eleValue;
        ...
        $newRow['LocalendTime'] = $row['LocalendTime'];

        return $newRow;
    }

    public static function filterAnalysisData($data) {
        ...
        foreach ($data as $row) {
            ...
            $result = DashHelper::validateAndSetAnalyzerRecordUsingRange($row, $acSettings);
    ...
    public static function getTonsTImeRange($maxTons, $startTime, $endTime) {


        $timesArray = array('LocalendTime' => $endTime, 'LocalstartTime' => $endTime);
        $whereClouse = "LocalendTime > '$startTime' AND LocalendTime < '$endTime'";
        $minDataIDQuery = "select min(dataID) as min from analysis_A1_A2_Blend where " . $whereClouse;

        $maxDataIDQuery = "select max(dataID) as max from analysis_A1_A2_Blend where " . $whereClouse;
        ...
        if (isset($maxDataID)) {
            ...
            while ($currentDataID >= $minDataID) {
                $currentDataID--;
                $result = Yii::app()->db->createCommand("select * from analysis_A1_A2_Blend where dataID = $currentDataID")->queryRow();

                if ($result) {

                    $timesArray['LocalstartTime'] = $result['LocalstartTime'];
                }

                if ($totalTons <= $maxTons) {
                    $totalTons += isset($result) ? $result['totalTons'] : 0;
                } else {
...
//        $elements[0] = 'LocalendTime';
        ...
        $tableTagHtml = '<table id="avg-table" class="customStyle dataTable no-footer" >';
        $tableHeadTagHtml = '<thead>';
...
//			if(substr($dele, -3, 3) == "_sp") continue;
            ...
            if ($dele == 'LocalendTime') {
                $tableHeadTagHtml .= "<th class='ui-state-default '>  " . Yii::t('app', 'LocalendTime') . " </th>";
            } else
                $tableHeadTagHtml .= "<th class='ui-state-default '>  " . $dele . " </th>";
        }//foreach

        $tableHeadTagHtml .= '</thead>';
        $tableBodyTagHtml = '<tbody>';
        ...
        $ri = 0;
        $dispAr = array();
        $acSettings = LabUtility::getOffsetRangeSettings();
        ...
        while ($i > $stopTime) {
            ...
            if (1):
                ...
                foreach ($elements as $ele) {
                    ...
                    if ($ele == "LocalendTime")
                        $rowTr .= "<td> " . date($dateTimeFormat, strtotime($avgArray[$ele])) . " </td>";
                    else
                        $rowTr .= "<td> " . round($avgArray[$ele], 2) . " </td>";
                }
        ...
        foreach ($dispAr as $erow) {
            $tableBodyTagHtml .= $erow;
        }

        $tableBodyTagHtml .= '</tbody>';

        return $tableTagHtml . $tableHeadTagHtml . $tableBodyTagHtml . '</table>';
    }

    public static function getaIntervalAvg($startDate, $endTime, $dataSrc, $elements) {

        if ($dataSrc == "FDR")
            $dTable = "rm_source_feeder_inputs";
        else
            $dTable = "analysis_A1_A2_Blend";

        $sql = "select * from $dTable where LocalendTime >= '" . $startDate . "'  AND LocalendTime <= '" . $endTime . "' and totalTons > 0 ORDER BY LocalendTime DESC";
        ...
        foreach ($result as $rawData) {
            ...
            $query = "select * from rm_settings where varKey = 'ANALYZER_FILTER_BAD_RECORDS' ";
            $result = Yii::app()->db->createCommand($query)->queryRow();
            if ($result && $result['varValue'] && ($dataSrc != "FDR"))
                $setting_filter_bad_records = (int) $result['varValue'];
            //Filter Here
            if ($setting_filter_bad_records)
            ...
            foreach ($elements as $ele) {
                ...
                if ($ele == 'KH') {
                    ...
                    if ($tSiO2 > 0)
                        $formulaVal = round(((4.07 * $tCaO) - (7.6 * $tSiO2) - (6.72 * $tAl2O3) - (1.43 * $tFe2O3) - (2.852 * $tSO3)), 2);
                    else
                        $formulaVal = 0.0;
                    $row[$ele] = $formulaVal;
                } else if ($ele == 'C3A') {
                    ...
                    if ($tAl2O3 > 0)
                        $formulaVal = round(((2.65 * $tAl2O3) - (1.69 * $tFe2O3)), 2);
                    else
                        $formulaVal = 0.0;
                    $row[$ele] = $formulaVal;
                } else if ($ele == 'LSF') {
                    ...
                    if ($tSiO2 > 0)
                        $formulaVal = round((($tCaO) / ((2.8 * $tSiO2) + (1.18 * $tAl2O3) - (0.65 * $tFe2O3))), 2);
                    else
                        $formulaVal = 0.0;
                    $row[$ele] = $formulaVal;
                } else if ($ele == "SM") {
            ...
            if (($dTable == "rm_source_feeder_inputs")) {
                $tsumFdr = round(($row["src_1_m"] + $row["src_2_m"] + $row["src_3_m"] + $row["src_4_m"] + $row["src_5_m"] + $row["src_6_m"]), 2);
                $average["R_TPH"][] = $tsumFdr;
            }
        }
...
//function
    ...
    public static function feedRateAverageTable($startTime = '', $endTime = '') {

        $defaultPrevHourLimit = 8;
        $defaultHour = 1 * 3600;
        ...
        $elements = DashHelper::getAvgElements(2);
        $displayElements = $elements;
        ...
        foreach ($displayElements as $item) {


            if (substr($item, 0, 4) == "src_") {
                $tsrcId = substr($item, 4, 1);
                $dele = (isset($srcArray[$tsrcId])) ? $srcArray[$tsrcId] : $item;
            ...
            $string1 = Yii::t('dash', $dele) . $suffix;
            $tableHeadTagHtml .= "<th class='ui-state-default '>  " . $string1 . " </th>";
        }//foreach
        ...
        while ($i > $stopTime) {
            ...
            $avgArray = DashHelper::getFeedRateIntervalAvg($sTime, $eTime);
            $avgArray['LocalendTime'] = $sTime;
    ...
    public static function getAvgElements($default = 1) {
    ...
    public static function getFeedRateIntervalAvg($startDate, $endTime) {



        $sql = "select * from rm_source_feeder_inputs where LocalendTime >= '" . $startDate . "'  AND LocalendTime <= '" . $endTime . "' ORDER BY LocalendTime DESC";
        ...
        foreach ($result as $row) {
            ...
            $src1m = $row['src_1_r'] == 1 ? $row['src_1_m'] : 0;

            $src2m = $row['src_2_r'] == 1 ? $row['src_2_m'] : 0;

            $src8m = $row['src_8_r'] == 1 ? $row['src_8_m'] : 0;

            $src3m = $row['src_3_r'] == 1 ? $row['src_3_m'] : 0;

            $src4m = $row['src_4_r'] == 1 ? $row['src_4_m'] : 0;

            $src5m = $row['src_5_r'] == 1 ? $row['src_5_m'] : 0;

            $src6m = $row['src_6_r'] == 1 ? $row['src_6_m'] : 0;

            $src7m = $row['src_7_r'] == 1 ? $row['src_7_m'] : 0;

            $rTPHArray[] = $src1m + $src2m + $src8m + $src3m + $src4m + $src5m + $src6m + $src7m;
        }


        for ($i = 1; $i <= 8; $i++) {

            $measuredArray['src_' . $i . '_m'] = LabUtility::getColumn($result, 'src_' . $i . '_m');

            $sparrayArray['src_' . $i . '_sp'] = LabUtility::getColumn($result, 'src_' . $i . '_sp');

            $runArray['src_' . $i . '_r'] = LabUtility::getColumn($result, 'src_' . $i . '_r');
        }

        //Filter 
        for ($i = 1; $i <= 8; $i++) {

            foreach ($runArray['src_' . $i . '_r'] as $index => $value) {

                if ($value == 0) {

                    unset($measuredArray['src_' . $i . '_m'][$index]);
                    unset($sparrayArray['src_' . $i . '_sp'][$index]);
                }
            }
        }
        ...
        for ($i = 1; $i <= 8; $i++) {
            ...
            $avg['src_' . $i . '_m'] = array_sum($measuredArray['src_' . $i . '_m']) / count($measuredArray['src_' . $i . '_m']);
            $avg['src_' . $i . '_sp'] = array_sum($sparrayArray['src_' . $i . '_sp']) / count($sparrayArray['src_' . $i . '_sp']);
        }


        $tsumFdr = round(array_sum($rTPHArray) / count($rTPHArray), 2);
        $avg["R_TPH"] = $tsumFdr;
