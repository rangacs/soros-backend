...
class ScaleParams {
    ...
    public function __construct($log = null) {
        ...
        $this->setting_TestingModeDB = $this->readSettingDB('AUTO_CALIB_TEST');
    ...
    function filterRecords($arLab, $arAnalyzer, $elementName, $settings) {
        ...
        foreach ($combineArray as $key => $item) {
            ...
            if ($value <= abs($ignore_offset_difference)) {
                $combineArray[$key]['valid'] = 0;
                $item['valid'] = 0;
    ...
    function calculateOffset($arLab, $arAnalyzer, $settings) {
        ...
        if ((!$anaSum || !$labSum) && !$this->setting_TestingModeDB) {
