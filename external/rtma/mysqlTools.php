...
function retrieveAllTagDataFromMYSQL(
    $db,                                         //!< database connection object
    $objIDName,                                  //!< Column name for the objectID
    $tablename,                                  //!< Table where the object information resides in database
    $objClass,                                   //!< PHP class name of object
    ...
    )
{
    ...
    $selectQry = "SELECT $objIDName FROM $tablename";
    if ($parentIDName != NULL && $parentID != NULL)
        $selectQry .= " WHERE $parentIDName = '$parentID'";
    if ($tagGroupID != NULL)
        $selectQry .= " AND tagGroupID = '$tagGroupID' ";

    $selectQry .= " ORDER BY  $objIDName $sortOrder";
    $dbResponse = $db->query($selectQry);
    if ( empty($dbResponse) || $dbResponse->num_rows == 0)
        return false;
    $outputArray = array();
...
}   // end retrieveAllTagDataFromMYSQL()
...
function mysql_table_check($db, $databaseName, $tableName)
{
/*  for helios
 ...
 */
    $selectQry = "show tables like '$tableName'";
    $result = $db->query($selectQry);
    if ($result === false)
        return false;
    $functionResult = $result->fetch_row();
    if ($functionResult[0] != $tableName)   // returns an enumerated array with one element which is a boolean indicating whether the function call was successful or not
        return false;
    return true;
    //$db->close();
...
$selectQry = "SELECT tagID from rta_tag_index_queued where tagGroupID = '" . $groupID . "' ";

     $dbres = $db->query($selectQry);
     if($dbres->num_rows == 0)
     {
        $selectQry = "SELECT tagID from rta_tag_index_completed where tagGroupID = '" . $groupID . "' ";
...
$selectQry = "SELECT *  from tag_group";
