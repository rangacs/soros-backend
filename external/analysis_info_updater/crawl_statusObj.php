...
function updateDetInfoToDb($dbCon, $colStr_s, $valStr_1_s, $valStr_2_s, $rmSettingsAr) {
    ...
    if(0){
        echo "det1 query: ".$ins1."<br/>\n";
        echo "det2 query: ".$ins2."<br/>\n";
    }
    ...
    if ($result)
        ...
        $err .= "DB analysis_status_info Det-1 Update Failed  !" . date("Y-m-d H:i:s") .  "<br/>\n";


    $result = mysqli_query($dbCon, $ins2);
    if ($result)
        $err .= " and Det-2 updated successfully at " . date("Y-m-d H:i:s") . " <br/>\n";
    else
...
//DetectorStatus constructor
    ...
    $aObj                          //!< Pre-run AnalysisObj with alignment info for all detectors.
        ...
        $detectorString = $this->detectorID->getValue();

        if (array_key_exists($detectorString, $aObj->issuedCmds))
            $issuedCmd = $aObj->issuedCmds[$detectorString];
        else
            return FALSE;

        $this->analyzeCmdIssued->setValue($issuedCmd);
...
class SystemStatus {
    ...
    function getStatus() {
        ...
        foreach ($this->detectorIDArray as $detID) {
            $result = $this->detectorStatus[$detID]->getStatus($aObj);
        }
